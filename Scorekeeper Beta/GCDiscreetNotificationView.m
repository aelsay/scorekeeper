...
- (CGPoint) hidingCenter {
    CGFloat y = 0;
    // The if statment below is/was modified by Ali ElSayed to better suit Scorekeeper.
    // In a TableView, the view is scrollable.
    // The notification hides just above the top bar (default value = -15) which will reveal it (the notification)
    // when the scroll view or tableview is scrolled down (that's if the view has no top nav bar)
    // In order to hide it, its y value must be even lower than -15.
    if (self.presentationMode == GCDiscreetNotificationViewPresentationModeTop) y = - 15;
